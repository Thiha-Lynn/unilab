...
export function renderInstall(app) {
  ...
  app.appendChild(el(`<main class="wrap privacy-page">
    ...
    <header class="hero"><p class="note">UniLab v${__APP_VERSION__}</p>
      ...
      <p>Use UniLab in your browser, install it on your home screen, or download a desktop app. Your files are processed locally in every version.</p>
    ...
    <div class="install-grid">
      ...
      <section class="install-card"><h2>Android</h2>
        ...
        <p class="note">Mobile web app. No APK or Play Store account is required.</p>
      </section>
    ...
    <section class="install-card"><h2>Ready before you go offline</h2>
      <p>On the <a href="#/">tools page</a>, choose <b>Make UniLab work offline</b> and wait for confirmation. Core tools use less than 8 MB. OCR and background removal need additional downloads while online. Desktop packages already include core tools.</p>
      ...
      <p class="note">Desktop builds are previews. Build checks and responsive browser checks do not mean every tool has been tested on every physical device. See the <a href="${release}" target="_blank" rel="noopener">release notes</a> for the current testing record.</p>
    </section>
  </main>`));
}
