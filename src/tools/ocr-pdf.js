...
export default function render(container, tool) {
  ...
  toolShell(container, tool, {
    ...
    note: 'Once the words are real text you can Ctrl+F a 60-page scanned handout for the one definition you need, and paste a quote into your report instead of retyping it. The language data is downloaded once and then kept by this browser, so the second scan you do today starts straight away.',
    ...
    options(host) {
      ...
      ui.download = infoBox(
        'The first run downloads the recognition engine (about 5 MB) and the language data '
    ...
    async run(ctx) {
      ...
      try {
        ...
        throw new Error('The recognition engine could not be downloaded. Check your connection — it is a one-time download of about 15 MB, and it needs to finish before any reading can start.');
      }
