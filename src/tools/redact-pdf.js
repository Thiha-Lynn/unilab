...
export default function render(container, tool) {
  ...
  async function exportFlattened(ctx, bytes) {
    ...
    const dpi = ui.dpi.value;
    const marked = new Set(state.boxes.map((b) => b.page));
    ...
    const passIdx = [];
    for (let n = 1; n <= state.pageCount; n++) if (!marked.has(n)) passIdx.push(n - 1);
    ...
    for (let n = 1; n <= state.pageCount; n++) {
      ...
      ctx.setBusy((n - 1) / state.pageCount, marked.has(n)
        ? `Rebuilding page ${n} of ${state.pageCount}…`
        : `Copying page ${n} of ${state.pageCount}…`);

      const passThrough = copiedBy.get(n - 1);
      if (passThrough) { out.addPage(passThrough); await tick(); continue; }

      const vp = (await state.pdf.getPage(n)).getViewport({ scale: 1 });
      const scale = Math.min(dpi / 72, RASTER_MAX_PX / Math.max(vp.width, vp.height));
      const canvas = await renderPage(state.pdf, n, scale);
      const c = canvas.getContext('2d');
      c.fillStyle = state.colour;
      for (const b of state.boxes) {
        if (b.page !== n) continue;
        c.fillRect(
          Math.round(b.x * canvas.width), Math.round(b.y * canvas.height),
          Math.round(b.w * canvas.width), Math.round(b.h * canvas.height),
        );
      }
