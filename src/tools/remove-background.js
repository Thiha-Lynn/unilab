...
export default function render(container, tool) {
  ...
  function modelGate() {
    ...
    const setLabel = () => {
      ...
      btn.textContent = !state.ready
        ? `Download the cut-out model (${formatBytes(downloadSize())}, once)`
  ...
  async function getCutout() {
    ...
    ui.gate.busy(true, 'Cancel the download', true);
  ...
  async function downloadModel(signal) {
    ...
    ui.gate.show(0, 'Starting the download…');
    try {
    ...
    ui.gate.show(1, 'Model downloaded.');
  }
  ...
  async function loadFile(file) {
    ...
    ui.gate?.status(state.ready
      ? 'The model is already downloaded. Press "Find the edges again".'
      : '');
    update();
  ...
  function currentLabel() {
    ...
    if (!state.mask) {
      return state.ready
        ...
        : 'This is the original. The cut-out appears after the model is downloaded.';
    }
  ...
  function friendlyError(err) {
    ...
    if (/fetch|network|Failed to fetch|metadata not found/i.test(msg)) {
      return 'The model could not be downloaded. Check your connection and press the button again — nothing of yours was sent.';
    }
