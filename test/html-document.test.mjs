...
// Optional local reference validation; copyrighted lecture files are not copied
// into the public repository. CI uses the synthetic equivalent above.
for (const [name, count] of [['W4-Class-2569.html',18],['W3-Class-2569.html',34]]) {
  let source; try { source = readFileSync(new URL(`../../${name}`,import.meta.url),'utf8'); } catch { continue; }
  test(`${name}: all ${count} lecture slides survive conversion`, () => {
    const p = prepareHtml(source,win);
    assert.equal(p.slideCount,count);
    assert.ok(p.text.length > 10000);
    assert.equal(new JSDOM(p.html).window.document.querySelectorAll('script').length,0);
  });
}
