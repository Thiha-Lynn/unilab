...
**Last updated:** 16 Sep 2026 (evidence pass — 15 interviews captured; F8/F11 parked) · **Owner:** Thiha Lin, Tech Lead
...
| **B17** | 🔴 Pending — owner will present to Dr. Prasara today (16 Sep); no approval or gate pass inferred |
...
|---|---|---|---|---|
| **B1** | Select a file and transform it entirely on-device; no upload path exists anywhere in the codebase | Must | F1, LR1, P2 | ✅ Done — all 59 tools |
...
| **B7** | Redact and blur so removed content is genuinely absent from the output file, not overlaid | Must | F7, LR6, P2 | ✅ Done — redaction export regression verifies absent text and output pixels, 16 Sep 2026 (B15) |
| **B9** | Cut the offline precache from **31 MB to ≤ 8 MB** by fetching the 23 MB ONNX runtime on demand instead of precaching it | Must | NFR3, D1, P3 | ✅ Done — 7.31 MB core, optional ONNX engine, build enforces 8,000,000 bytes; offline conversion verified 16 Sep |
...
| **B11** | Write the test that proves NFR1: run all 59 tools against sample files, assert **0 bytes of file content** in any request body | Must | NFR1, LR1, D5, P2 | ⚠️ Partial — all 59 routes mount; full HTML conversion has zero upload requests. Processing sweep across every tool remains open |
...
| **B15** | QA test for B7: extract text from a redacted PDF and assert the redacted string is absent | Must | LR6, F7, P2 | ✅ Done — `test/redaction-output.test.mjs`: real PDF render, removed text, output pixel and untouched-text checks |
...
| **B21** | Installable PWA that works with no connection | Could | F13, P3 | ✅ Done — core below 8 MB; browser-offline HTML conversion verified 16 Sep; optional AI downloads disclosed |
...
| **B17** | Confirm with Dr. Prasara that UniLab is an approved case-study topic (not on the 19 Aug assigned list) | Must | Rubric — *topic* · spec Q4 | 🔴 Pending — owner will present to Dr. Prasara today (16 Sep); no approval or gate pass inferred |
...
| ✅ Done | 23 |
| ⚠️ Partial / unverified | 2 (B11, B35) |
| 🔴 To do | 2 (B33, B34) |
...
Run `/audit-backlog` before any commit touching `.docs/` and before the gate submission. The 8 Sep 2026 historical report recorded **four of four checks clean** over the product table, and lists the two
parked rows by name rather than counting them as failures — that is not a false positive, it is
Q5/Q6.

## Week 6 update — 16 Sep

Current scope, CR1 (HTML converter), release workflow, evidence and unresolved
measurements are in [Week 6 release record](../03-build/20260916-week6.md).
The original interview-derived table remains distinct from this owner request.
B17 remains pending today’s presentation. OCR/workflows remain parked for course
commitment; existing functionality is retained.

The 16 Sep rule-level audit records a pre-existing legal-coverage gap; see
[20260916 log](../05-log/20260916-log.md). Do not present the historical pass as the current result.
